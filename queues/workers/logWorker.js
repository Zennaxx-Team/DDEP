...
const { blukSave } = require('../../controllers/log_history.controller');
...
async function updateLogHistoryBatch(logs) {
    try {
        await axios.put(`${config.domain}/logs/update/missingId/bulk`, { logs });
        console.log(`Bulk backfill success for ${logs.length} logs`);
    } catch (err) {
        console.error(`Bulk backfill failed: ${err.message}`);
    }
}

// Create batch queue
const batchQueue = new Queue(`${logQueueConfig.name_complate}`, { connection });

// Batch worker - processes batches of 50
const batchWorker = new Worker(`${logQueueConfig.name_complate}`, async (job) => {
    const logs = job.data.logs;
    console.log(`Processing batch of ${logs.length} logs`);

    try {
        const result = await blukSave(logs);
        if (result.status === 1) {
            console.log(`[BATCH] Successfully processed ${logs.length} logs`);
            await updateLogHistoryBatch(logs);
        } else {
...
const worker = new Worker(logQueueConfig.name_save, async (job) => {
...
async function createBatch() {
    ...
    try {
        ...
        const logQueue = new Queue(logQueueConfig.name_save, { connection });
