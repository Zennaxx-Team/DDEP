...
const { bulkSaveDiffHistory } = require('../../controllers/diff_history.controller');
...
// Create batch queue
const batchQueue = new Queue(`${logQueueConfig.name_diff_complate}`, { connection });

// Batch worker - processes batches of 50
const batchWorker = new Worker(`${logQueueConfig.name_diff_complate}`, async (job) => {
    const logs = job.data.logs;
    console.log(`Processing batch of ${logs.length} logs`);

    try {
        const result = await bulkSaveDiffHistory(logs);
...
const worker = new Worker(logQueueConfig.name_diff_save, async (job) => {
...
async function createBatch() {
    ...
    try {
        ...
        const logQueue = new Queue(logQueueConfig.name_diff_save, { connection });
