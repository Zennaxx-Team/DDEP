...
let responseData = [];
let alertDebugFromDate = null,
    alertDebugToDate = null;


$(document).ready(async function () {
    if (!$('#alert-debug-history-data-table').length) return;
    ...
    $('#alertdebugreportrange').daterangepicker({
        timePicker: true,
    ...
    function updateDisplay(start, end, matchedLabel) {
        if (matchedLabel) {
            ...
            $('#alertdebugreportrange span').text(matchedLabel);
            $('.since-time-alert-debug').text(matchedLabel);
            ...
            $('#alertdebugreportrange span').text(displayText);
            $('.since-time-alert-debug').text('Custom Range');
        }
    }

    function dateCallback(start, end) {
        alertDebugFromDate = start.toISOString();
        alertDebugToDate = end.toISOString();
    ...
    $('#alertdebugreportrange').on('apply.daterangepicker', function (ev, picker) {
    ...
    function initAlertHistoryTable() {
        if ($.fn.DataTable.isDataTable('#alert-debug-history-data-table')) return;

        table = $('#alert-debug-history-data-table').DataTable({
            paging: false,
            searching: true,
            info: false,
            ordering: false,
            ...
            initComplete: function () {
                $('#alert-debug-history-data-table_filter input')
                    .off()
                    .on('keyup', function (e) {
                        if (e.keyCode === 13) {
                            alertHistoryCurrentPage = 1;
     ...
     function getAlertHistorycondition() {
        ...
        if (!alertDebugFromDate || !alertDebugToDate) return;

        $('#alert-debug-history-data-table tbody').html(
            '<tr><td colspan="11"><div class="tableloader"></div></td></tr>'
        );

        $.ajax({
            url: '/alerts/alert-history-debug/list',
            method: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
                page: alertHistoryCurrentPage,
                limit: alertHistoryPerPage,
                search: $('#alert-debug-history-data-table_filter input').val(),
                fromDate: alertDebugFromDate,
                toDate: alertDebugToDate
            }),
    ...
    function renderPagination(total) {
        ...
        if (totalPages <= 1) {
            $('#alert-debug-history-data-table_paginate').html('');
            return;
        }
        ...
        $('#alert-debug-history-data-table_paginate').html(html);
    }
    ...
    $('body').on('change', '#alert-debug-history-data-table_length select', function () {
...
async function getDetailAlertHistorycondition(alertDetailPerPage, alertDetailCurrentPage, uniqueAlertId) {
    ...
    $.ajax({
        url: '/alerts/alert-history-debug/get/' + uniqueAlertId,
        method: 'POST',
        contentType: 'application/json',
...
function viewAlertHistoryDetails(id) {
    ...
    uniqueAlertId = id;
    $.ajax({
        url: '/alerts/alert-history-debug/' + id,
        method: 'GET',
        success: function (response) {
