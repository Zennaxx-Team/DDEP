...
var propsValidationRowCounter = 1;
var propsFormatRowCounter = 1;
...
function mappingformuladatafunc(itemKey, itemName, linkedItems) {
	...
	if (itemProperties.length > 0) {
		for (var i = 0; i < itemProperties.length; i++) {
			if (itemProperties[i].general.itemKey == itemKey) {
				var itemFormatAdditionalRules = itemProperties[i].format.additonal_rules;
				...
				if (itemProperties[i].validation.additonal_rules != undefined) {
					var propValidationAdditonalRules = itemProperties[i].validation.additonal_rules;
					for (var j = 0; j < propValidationAdditonalRules.length; j++) {
						...
						propValidationAdditionalRulesTr += '</select></td><td class="col-sm-3 autocomplete"><input type="text" name="propsvalidations[][original]" class="form-control border-0 autocompleteformula" id="proporiginal_' + j + '" value="' + propValidationAdditonalRules[j].original + '"/></td><td class="col-sm-3 operations-select"><select class="select-dropdown form-control form-control-lg" name="propsvalidations[][operations]"><option value="=="';
						...
						propValidationAdditionalRulesTr += '>Contains</option></select></td><td class="col-sm-2 autocomplete"><input type="text" name="propsvalidations[][column]" class="form-control border-0 autocompleteformula" id="propcolumn_' + j + '" value="' + propValidationAdditonalRules[j].column + '"/></td><td class="col-sm-2"><select class="select-dropdown form-control form-control-lg" name="propsvalidations[][then]"><option value="STOP"';
				...
				if (itemFormatAdditionalRules != undefined) {
					...
					for (var l = 0; l < propFormatAdditonalRules.length; l++) {
						...
						propFormatAdditionalRulesTr += "<input type='text' name='formatrules[][formulato]' class='form-control border-0 autocompleteformula' id='format-additional-rules-formulato-" + l + "' value='" + propFormatAdditonalRules[l].formulato + "' ";
						...
						if (propFormatAdditonalRules[l].name == 'REPLACE' || propFormatAdditonalRules[l].name == 'SUBSTRING') {
							propFormatAdditionalRulesTr += "<input type='text' name='formatrules[][formulatonew]' class='form-control border-0' id='format-additional-rules-formulatonew-" + l + "' value='" + propFormatAdditonalRules[l].formulatonew + "'/>";
						}
	...
	$('#mapping-formula-props #props-validation-is-required > option[value="' + validationIsRequired + '"]').attr('selected', 'selected').prop('selected', true);

	$('#mapping-formula-props #props-validation-value-must-be > option[value="' + validationValueMustbe + '"]').attr('selected', 'selected').prop('selected', true);
	...
	$('#mapping-formula-props #props-format-is-trim > option[value="' + formatTrim + '"]').attr('selected', 'selected').prop('selected', true);

	$('#mapping-formula-props #props-format-enable-rounding > option[value="' + formatEnableRounding + '"]').attr('selected', 'selected').prop('selected', true);

	$('#mapping-formula-props #props-format-enable-decimal > option[value="' + formatEnabeDecimal + '"]').attr('selected', 'selected').prop('selected', true);
...
$('body').on("click", "#OutboundDataBind", function() {
